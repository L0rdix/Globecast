...
function GetWeather() {
  ...
  fetch(
    `https://api.openweathermap.org/data/2.5/weather?q=${inputVal}&APPID=692aecf0eca12b1e8b1b72c5e62db76f&units=metric`
  )
    ...
    .then((data) => {
      ...
      if (listItemsArray.length > 0) {
        ...
        if (arrayFilter.length > 0) {
          errorMsg.innerHTML = `You already know the weather for ${
            arrayFilter[0].querySelector(".city-name span").textContent
          }. Try searching for another city or provide a country code.`;
          inputElement.value = "";
          return;
        }
      }
